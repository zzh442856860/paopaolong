...
bool GameScene::isCollisionWithBorder()	//判断发射的球球，是否和左右边缘碰撞//
{
	CCSize size = CCDirector::sharedDirector()->getWinSize();
...
ROWCOL_LIST GameScene::findClearBubble(Bubble *pReadyBubble)
{
	...
	else if (pReadyBubble->getBubbleColor() == COLOR_COUNT)//怎么会变成这个值？color会变化？//
	{
		return clearRowCollist;
	}
	else
	{
		...
		if (clearRowCollist.size() < REMOVE_COUNT)//3个才可以消除//
		{
